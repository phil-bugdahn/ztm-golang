...
func runJob(i Job, resultChannel chan<- int) {
...
func getTotal(resultChannel chan int) int {
	total := 0
	for {
		select {
		case jobResult := <- resultChannel:
			total += jobResult
...
func main() {
	...
	for i := 0; i < len(jobs); i++ {
		go runJob(jobs[i], resultChannel)
	}

	total := getTotal(resultChannel)
